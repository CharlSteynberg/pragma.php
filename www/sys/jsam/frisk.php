...
// get syntax config (only for test mode)
// --------------------------------------------------------------------------------------
   if (liveMode === false)
   { jsam::set('conf.syntax', parse::file('cfg/jsam/parse.syntax.json')); }
// --------------------------------------------------------------------------------------



// minify (and optionally check syntax errors in test mode)
// --------------------------------------------------------------------------------------
   Jsam::set('frisk', function($jd, $fn=null)
   {
   // validate
   // -----------------------------------------------------------------------------------
      if (typeOf($jd) !== str)
      ...
      if (substr(CWD.'/'.$jd, -4, 4) == 'jsam')
      {
         $tp = CWD.'/'.$jd;

         if (!is_readable($tp))
         {
            $m = (!file_exists($tp) ? 'undefined' : 'forbidden');
            throw new Exception('"'.$jd.'" is '.$m);
         }

         $fn = $jd;
         $jd = file_get_contents($tp);
      }
      ...
      if (strlen(trim($jd)) < 1)
      { throw new Exception('"'.$fn.'" is empty'); }
   // -----------------------------------------------------------------------------------


   // constants (never changes during runtime)
   // -----------------------------------------------------------------------------------
      $sx = ((liveMode === false) ? Jsam::get('conf.syntax') : null);

      $qb = chr(171);                                 // quote bgn
      $qe = chr(187);                                 // quote end
...
//            if ((($pr == 'pt') && (strpos('dn ft', $cr) !== false)) || (($pr == 'dn') && ($cr == 'pt')))
//            { $cr = 'pt'; }
         ...
         // -----------------------------------------------------------------------------
            if ($sc === false)
            {
               ...
               if ($er > 0)
               { fail::syntax('unexpected "'.$rd->$cr.'" {'.$rp.'}', $fn, $lc[0].':'.$lc[1]); }
            }
         // -----------------------------------------------------------------------------
         }
      // --------------------------------------------------------------------------------
